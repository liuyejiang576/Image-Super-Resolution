#!/usr/bin/env python3
"""Train ECBSR on DIV2K with periodic validation (B3).

Called by `run_ecbsr_20k.py` via `ecbsr_20k.py resume`. Prefer:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", default="configs/train_ecbsr.yaml")
...
def main() -> None:
    ...
    aux_weight = float(train_cfg.get("aux_weight", 0.0))
    use_aux = (
        aux_weight > 0.0
        and bool(getattr(model, "supports_aux_loss", False))
    )
    if aux_weight > 0.0 and not getattr(model, "supports_aux_loss", False):
        raise ValueError(
            f"aux_weight={aux_weight} set but model {type(model).__name__} "
            "does not support aux loss"
        )
    optimizer = Adam(
        model.parameters(),
    ...
    for epoch in range(start_epoch, train_cfg["epochs"] + 1):
        ...
        for step, batch in enumerate(train_loader, start=1):
            ...
            with torch.autocast(device_type=device.type, enabled=scaler.is_enabled()):
                if use_aux:
                    pred, pred_aux = model(lr, return_aux=True)
                    pred = _match_size(pred, hr)
                    pred_aux = _match_size(pred_aux, hr)
                    loss = criterion(pred, hr) + aux_weight * criterion(pred_aux, hr)
                else:
                    pred = _match_size(model(lr), hr)
                    loss = criterion(pred, hr)
        ...
        log_row = {
            ...
            "elapsed_sec": elapsed,
            "loss_type": loss_type,
            "aux_weight": aux_weight if use_aux else 0.0,
        }
