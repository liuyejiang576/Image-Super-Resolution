#!/usr/bin/env python3
"""A1a FSRCNN bs24 20k control: pause | resume | watch.

  python scripts/a1a_20k.py pause [--dry-run]
  python scripts/a1a_20k.py resume [--dry-run]
  python scripts/a1a_20k.py watch [--interval 60]

Launcher (run_a1a_20k.py) is started by resume only.
...
MANIFEST = ROOT / "results/exp_runs/a1a_20k_manifest.json"
PAUSE_STATE = ROOT / "results/exp_runs/a1a_20k_paused.json"
LOG_DIR = ROOT / "results/exp_runs/logs"
RUN_IDS = ["fsrcnn_fix_clean_20k_bs24"]
...
def train_pids(run_id: str) -> list[str]:
    ...
    return pgrep(f"train_fsrcnn.py --config {cfg}")


def launcher_pids() -> list[str]:
    return pgrep("run_a1a_20k.py")


def watch_pids() -> list[str]:
    return [p for p in pgrep("a1a_20k.py watch") if p != str(os.getpid())]
...
def run_info(run_id: str) -> dict:
    ...
    return {
        "run_id": run_id,
        "variant": meta.get("variant", "A1a_bs24"),
        "state": state,
        "epoch": epoch,
        "target": target,
        "psnr": psnr,
...
def render_watch(infos: list[dict]) -> None:
    ...
    lines = [f"A1a FSRCNN bs24 20k (A1a) — {now}", ""]
    hdr = (
        f"{'run_id':<22} {'state':<8} {'epoch':>11} {'step':>10} {'val_psnr':>9} "
        f"{'best':>9} {'spent':>9} {'ETA':>9} {'prog':>6}"
    )
    ...
    for i in infos:
        ...
        lines.append(
            f"{i['run_id']:<22} {i['state']:<8} {ep_str:>11} {step_str:>10} {psnr:>9} "
            f"{best:>9} {fmt_dur(i['spent_sec']):>9} {fmt_dur(i['eta_sec']):>9} {i['progress_pct']:>5.1f}%"
        )
    lines.extend(
        [
            "",
            "Commands:  python scripts/a1a_20k.py pause | resume | watch",
            "Contract:  IMPLEMENTATION.md §1–§2 (epoch-granular checkpoints)",
        ]
    )
    if infos and infos[0]["state"] == "done":
        lines.append("Done — next: eval + fair-table side note (keep old bs=8 row).")
    print("\n".join(lines))


def cmd_pause(dry_run: bool) -> None:
    print(f"Pausing A1a 20k — {datetime.now():%Y-%m-%d %H:%M:%S}")
    state = {
    ...
    print("\nPaused. Checkpoint: results/exp_runs/fsrcnn_fix_clean_20k_bs24/checkpoints/latest.pt")
    print("Resume: python scripts/a1a_20k.py resume")


def cmd_resume(dry_run: bool) -> None:
    print(f"Resuming A1a 20k — {datetime.now():%Y-%m-%d %H:%M:%S}")
    any_pending = False
    for rid in RUN_IDS:
        ...
        if ep > 0 and not ckpt.exists():
            ...
            continue
        print(f"  {rid}: will run ({ep}/{target}) variant={meta.get('variant', 'A1a_bs24')}")
        any_pending = True
    ...
    if launcher_pids():
        ...
        log_file = open(LOG_DIR / "a1a_20k_launcher.log", "a", encoding="utf-8")
        proc = subprocess.Popen(
            [PY, "scripts/run_a1a_20k.py", "--skip-done"],
            cwd=ROOT,
            stdout=log_file,
    ...
    print("Monitor: python scripts/a1a_20k.py watch --interval 60")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="A1a FSRCNN bs24 20k: pause | resume | watch")
