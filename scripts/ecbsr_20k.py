#!/usr/bin/env python3
"""ECBSR-M10C16 20k control: pause | resume | watch.

  python scripts/ecbsr_20k.py pause [--dry-run]
  python scripts/ecbsr_20k.py resume [--dry-run]
  python scripts/ecbsr_20k.py watch [--interval 60]

Launcher (run_ecbsr_20k.py) is started by resume only.
...
MANIFEST = ROOT / "results/exp_runs/ecbsr_20k_manifest.json"
PAUSE_STATE = ROOT / "results/exp_runs/ecbsr_20k_paused.json"
LOG_DIR = ROOT / "results/exp_runs/logs"
RUN_IDS = ["ecbsr_m10c16_20k"]
...
def train_pids(run_id: str) -> list[str]:
    ...
    return pgrep(f"train_ecbsr.py --config {cfg}")


def launcher_pids() -> list[str]:
    return pgrep("run_ecbsr_20k.py")


def watch_pids() -> list[str]:
    return [p for p in pgrep("ecbsr_20k.py watch") if p != str(os.getpid())]
...
def run_info(run_id: str) -> dict:
    ...
    return {
        "run_id": run_id,
        "variant": meta.get("variant", "M10C16"),
        "state": state,
        "epoch": epoch,
        "target": target,
        "psnr": psnr,
...
def render_watch(infos: list[dict]) -> None:
    ...
    lines = [f"ECBSR-M10C16 20k (B3) — {now}", ""]
    hdr = (
    ...
    lines.extend(
        [
            "",
            "Commands:  python scripts/ecbsr_20k.py pause | resume | watch",
            "Contract:  IMPLEMENTATION.md §1–§2 (epoch-granular checkpoints)",
        ]
    )
    if infos and infos[0]["state"] == "done":
        lines.append("Done — next: fuse_ecbsr → convert/bench phone (B3 remaining).")
    print("\n".join(lines))


def cmd_pause(dry_run: bool) -> None:
    print(f"Pausing ECBSR 20k — {datetime.now():%Y-%m-%d %H:%M:%S}")
    state = {
    ...
    print("\nPaused. Checkpoint: results/exp_runs/ecbsr_m10c16_20k/checkpoints/latest.pt")
    print("Resume: python scripts/ecbsr_20k.py resume")


def cmd_resume(dry_run: bool) -> None:
    print(f"Resuming ECBSR 20k — {datetime.now():%Y-%m-%d %H:%M:%S}")
    any_pending = False
    for rid in RUN_IDS:
        ...
        if ep > 0 and not ckpt.exists():
            ...
            continue
        print(f"  {rid}: will run ({ep}/{target}) variant={meta.get('variant', 'M10C16')}")
        any_pending = True
    ...
    if launcher_pids():
        ...
        log_file = open(LOG_DIR / "ecbsr_20k_launcher.log", "a", encoding="utf-8")
        proc = subprocess.Popen(
            [PY, "scripts/run_ecbsr_20k.py", "--skip-done"],
            cwd=ROOT,
            stdout=log_file,
    ...
    print("Monitor: python scripts/ecbsr_20k.py watch --interval 60")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="ECBSR-M10C16 20k: pause | resume | watch")
