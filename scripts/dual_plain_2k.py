#!/usr/bin/env python3
"""DualStream / Plain C20N5 2k control: pause | resume | watch (B4 round-2).

Default resume trains **both** probes sequentially (Dual then Plain).

  python scripts/dual_plain_2k.py pause [--dry-run]
  python scripts/dual_plain_2k.py resume [--dry-run]
...
MANIFEST = ROOT / "results/exp_runs/dual_plain_2k_manifest.json"
PAUSE_STATE = ROOT / "results/exp_runs/dual_plain_2k_paused.json"
...
def resolve_run_ids(run_id: str | None) -> list[str]:
...
def train_pids(run_id: str) -> list[str]:
    ...
    # train_dual_plain.py runpy→train_ecbsr.py; match both process names.
    return (
        pgrep(f"train_dual_plain.py --config {cfg}")
        + pgrep(f"train_ecbsr.py --config {cfg}")
    )


def launcher_pids() -> list[str]:
    return pgrep("run_dual_plain_2k_probes.py") + pgrep("run_dual_plain_2k.py")


def watch_pids() -> list[str]:
    return [p for p in pgrep("dual_plain_2k.py watch") if p != str(os.getpid())]
...
def run_info(run_id: str) -> dict:
    ...
    return {
        ...
        "eta_sec": eta,
        "global_step": gstep,
        "updates_target": int(meta.get("updates_target", 2013)),
...
def render_watch(infos: list[dict]) -> None:
    ...
    lines = [f"DualStream / Plain C20N5 2k (B4 round-2) — {now}", ""]
    hdr = (
        f"{'run_id':<24} {'var':<12} {'state':<8} {'epoch':>11} {'step':>10} "
    ...
    for i in infos:
        ...
        lines.append(
            f"{i['run_id']:<24} {i['variant']:<12} {i['state']:<8} {ep_str:>11} {step_str:>10} "
    ...
    lines.extend(
        [
            "",
            "Commands:  python scripts/dual_plain_2k.py pause | resume | watch",
            "Default:   resume trains Dual then Plain; --run-id for one",
            "Gate:      D18 — Dual−Plain ≳0.03 dB; vs ECBSR@2k ≲0.10 dB; phone > E_med",
            "Contract:  IMPLEMENTATION.md §1–§2",
        ]
    )
    print("\n".join(lines))
...
def cmd_pause(dry_run: bool, run_ids: list[str]) -> None:
    print(f"Pausing Dual/Plain 2k — {datetime.now():%Y-%m-%d %H:%M:%S}")
    state = {
    ...
    print("\nPaused.")
    print("Resume: python scripts/dual_plain_2k.py resume")


def cmd_resume(dry_run: bool, run_ids: list[str]) -> None:
    print(f"Resuming Dual/Plain 2k — {datetime.now():%Y-%m-%d %H:%M:%S}")
    any_pending = False
    for rid in run_ids:
    ...
    if launcher_pids():
        ...
        log_file = open(LOG_DIR / "dual_plain_2k_launcher.log", "a", encoding="utf-8")
        cmd = [PY, "scripts/run_dual_plain_2k_probes.py", "--skip-done"]
        if len(run_ids) == 1:
    ...
    print("Monitor: python scripts/dual_plain_2k.py watch --interval 60")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Dual/Plain C20N5 2k: pause | resume | watch")
    ...
    p_resume = sub.add_parser("resume", help="Start launcher (Dual then Plain by default)")
    ...
    args = ap.parse_args()
    run_ids = resolve_run_ids(getattr(args, "run_id", None))
    if args.cmd == "pause":
        pause_ids = run_ids if args.run_id else list(manifest_by_id())
        cmd_pause(args.dry_run, pause_ids)
    elif args.cmd == "resume":
        cmd_resume(args.dry_run, run_ids)
    elif args.cmd == "watch":
        watch_ids = run_ids if args.run_id else list(manifest_by_id())
